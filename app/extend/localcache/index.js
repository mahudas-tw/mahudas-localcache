...
*/
const createPool = (name, { maxSize, defaultTTL, strategy } = {}) => {
  const _pool = _cachePools.get(name);
  // 如果原本就有這個pool，覆蓋options，回傳既有的pool
  if (_pool) {
    _pool.setOptions({ maxSize, defaultTTL, strategy });
    return _pool;
  }

  const _instance = new CachePool({ maxSize, defaultTTL, strategy });
